...
check_tun(){
    ...
    if [[ ! $TUN =~ 'in bad state' ]] && [[ ! $TUN =~ '处于错误状态' ]] && [[ ! $TUN =~ 'Die Dateizugriffsnummer ist in schlechter Verfassung' ]]; then
        if [[ $vpsvirt == "openvz" ]]; then
            wget -N --no-check-certificate https://raw.githubusercontent.com/Dicksoi/tun-script/master/tun.sh && bash tun.sh
        else
...
install_wireguard(){
    ...
    if [[ $vpsvirt =~ lxc|openvz ]]; then
        wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/Dicksoi/warp-script/files/wireguard-go -O /usr/bin/wireguard-go
        chmod +x /usr/bin/wireguard-go
    fi
    if [[ $vpsvirt == zvm ]]; then
        wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/Dicksoi/warp-scriptfiles/wireguard-go-s390x -O /usr/bin/wireguard-go
        chmod +x /usr/bin/wireguard-go
    fi
}

install_wgcf(){
    wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/Dicksoi/warp-script/files/wgcf_latest_linux_$(archAffix) -O /usr/local/bin/wgcf
...
generate_wgcf_config(){
    ...
    sed -i "9 s/^/PostUp = ip -6 rule add from $(ip route get 2606:4700:4700::1001 | grep -oP 'src \K\S+') lookup main\n/" wgcf-profile.conf
    sed -i "10 s/^/PostDown = ip -6 rule delete from $(ip route get 2606:4700:4700::1001 | grep -oP 'src \K\S+') lookup main\n/" wgcf-profile.conf
