...
check_tun(){
    ...
    if [[ ! $TUN =~ "in bad state"|"处于错误状态"|"ist in schlechter Verfassung" ]]; then
        if [[ $vpsvirt == lxc ]]; then
            ...
            wget -N --no-check-certificate https://gitlab.com/Dicksoi/tun-script/-/raw/master/tun.sh && bash tun.sh
        else
            red "检测到未开启TUN模块, 请到VPS厂商的控制面板处开启"
            exit 1
        fi
...
install_wgcf(){
    ...
    if [[ $SYSTEM == "CentOS" ]]; then
        ...
        if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]]; then
            if [[ $vpsvirt =~ "kvm"|"xen"|"microsoft"|"vmware"|"qemu" ]]; then
                wget -N --no-check-certificate https://gitlab.com/Dicksoi/warp-script/-/raw/master/files/wireguard-go-$(archAffix) -O /usr/bin/wireguard-go
                chmod +x /usr/bin/wireguard-go
            fi
        fi
        if [[ $vsid == 9 ]] && [[ -z $(type -P resolvconf) ]]; then
            wget -N https://gitlab.com/Dicksoi/warp-script/-/raw/master/files/resolvconf -O /usr/sbin/resolvconf
    ...
    wget -N --no-check-certificate https://gitlab.com/Dicksoi/warp-script/-/raw/master/files/wgcf-latest-linux-$(archAffix) -O /usr/local/bin/wgcf
...
install_wireproxy(){
    ...
    wget -N https://gitlab.com/Dicksoi/warp-script/-/raw/master/files/wireproxy-$(archAffix) -O /usr/local/bin/wireproxy
